...
export function isPlausibleLevel(level) {
...
export function startExpoCapture(options) {
    ...
    const clear = options.clearInterval ?? ((h) => clearInterval(h));
    let active = true;
    const handle = start(() => {
        if (!active)
            return;
        void Promise.resolve(options.recorder.getLevelDbfs()).then((level) => {
            if (!active || !isPlausibleLevel(level))
                return;
