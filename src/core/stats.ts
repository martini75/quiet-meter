 ...
 */
export function exceedance(levels: readonly number[], percent: number): number {
 ...
 */
export function leq(levels: readonly number[]): number {
  if (levels.length === 0) return Number.NaN;
  let energy = 0;
  for (const level of levels) energy += 10 ** (level / 10);
  return 10 * Math.log10(energy / levels.length);
}

export function summarise(levels: readonly number[]): LevelStats {
  if (levels.length === 0) {
